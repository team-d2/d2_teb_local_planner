...
TEST(TEBBasic, autoResizeLargeValueAtEnd)
{
  ...
  d2_teb_local_planner::TimedElasticBand teb;
  
  teb.addPose(d2_teb_local_planner::PoseSE2(0., 0., 0.));
  for (int i = 1; i < 10; ++i) {
    teb.addPoseAndTimeDiff(d2_teb_local_planner::PoseSE2(i * 1., 0., 0.), dt);
  }
  // add a pose with a large timediff as the last one
  teb.addPoseAndTimeDiff(d2_teb_local_planner::PoseSE2(10., 0., 0.), dt + 2*dt_hysteresis);
